...
export function AuthGate({ children }: PropsWithChildren) {
  ...
  if (!supabaseConfigured) return <main className="flex min-h-screen items-center justify-center bg-ink px-5 text-cream"><p className="max-w-lg rounded-md border border-line bg-panel p-5 text-sm">Supabase is not configured. Set VITE_SUPABASE_URL and VITE_SUPABASE_PUBLISHABLE_KEY.</p></main>
  if (!ready) return <AuthLoading />
  if (user && mode !== 'update') return children
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-ink px-5 py-10 text-cream">
      <section className="w-full max-w-sm rounded-xl border border-line bg-panel p-6 shadow-[var(--toast-shadow)]">
        ...
        <form className="mt-6 space-y-4" onSubmit={submit}>
          ...
          {mode !== 'forgot' && <label className="block text-xs text-dim">Password<input required minLength={8} type="password" autoComplete={mode === 'login' ? 'current-password' : 'new-password'} value={password} onChange={(e) => setPassword(e.target.value)} className="mt-1.5 w-full rounded-md border border-line bg-ink px-3 py-2.5 text-sm text-cream outline-none focus:border-mark" /></label>}
